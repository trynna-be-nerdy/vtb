...
**meetings** — one row per board meeting, contains Gemma 4 generated overview (meeting_date, title, source_url, meeting_overview, top_decisions[], fiscal_total, next_meeting_notes, total_items, fiscal_items, processing_status)
...
```json
{
  "primary_category": "one of 12 slugs below",
...
- Meeting cards: source badge, date, title, 2-sentence summary, category chip, urgency border color, fiscal icon, source PDF link
...
- Supporting documents section with type, summary, PDF link

### Category Pages (`/category/{slug}`)
- 12 sections, one per category
- Paginated item feed
- Sub-tag filter bar
...
1. **Scraper** — hits lcps.org (BeautifulSoup4) and loudoun.gov (Legistar JSON API) to collect PDF links
2. **Deduplicator** — checks `seen_documents` table by URL + SHA-256; skips unchanged documents
3. **Downloader** — fetches new PDFs via httpx, validates MIME type
4. **Extractor** — pdfplumber extracts text page-by-page
5. **Chunker** — splits on agenda item headers (`AGENDA ITEM`, `ITEM N`, `PUBLIC HEARING`), caps at 2,000 chars
6. **Gemma 4 Pass 1** — Prompt 1 per chunk: content rewrite
7. **Gemma 4 Pass 2** — Prompt 2 on rewritten summary: classification
8. **Validator** — 10 Python quality checks: summary length ≥ 80 chars, valid category slug, valid urgency, fiscal_impact is boolean, decisions is list, no N/A placeholders, etc.
9. **DB write** — atomic PostgreSQL transaction, search_vector trigger fires
10. **Cache invalidation** — Redis keys for affected category feeds and meeting detail deleted
...
- 4-second delay between requests, never parallel
- Legistar API for Loudoun County BOS (structured JSON — no scraping needed)
- Playwright fallback if basic requests are blocked
- ScraperAPI backup (1,000 req/month free tier)
...
│   │   └── services/            # Redis cache + WebSocket manager
│   ├── pipeline/
│   │   ├── worker.py            # APScheduler entry point
│   │   ├── scraper.py           # lcps.org + Legistar API
│   │   ├── extractor.py         # PDF download + pdfplumber
│   │   ├── chunker.py           # Agenda item splitter
│   │   ├── gemma.py             # 3 Ollama prompts + retry logic
│   │   ├── validator.py         # 10-check quality gate
│   │   └── publisher.py         # DB write + Redis + WS broadcast
│   ├── alembic/                 # DB migrations
│   ├── requirements.txt
│   └── Dockerfile
├── frontend/
│   ├── app/
│   │   ├── layout.tsx           # Root layout with Navbar + CategoryTicker
│   │   ├── page.tsx             # Homepage
│   │   ├── meetings/[id]/       # Meeting detail page
│   │   ├── category/[slug]/     # Category feed page
│   │   └── search/              # Search page
│   ├── components/
...
1. Get Gemma 4 returning clean JSON from a real LCPS meeting chunk — **this is the real unblock**
2. PDF extractor + chunker working on one real document
3. PostgreSQL schema initialized, SQLAlchemy models written, Alembic migrations run
...
9. Docker Compose file — so judges can run everything in one command
10. Demo video — real PDF in → Gemma 4 rewrites it → blog card live on public URL
11. README complete — architecture, screenshots, video link, setup commands
12. Kaggle Notebook technical write-up
13. Submit by May 18, 2026 at 11:59 PM UTC
...
| Page load time | Meeting feed renders in under 2 seconds |
